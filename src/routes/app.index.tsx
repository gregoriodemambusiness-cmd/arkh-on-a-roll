    ...
    <div className="mx-auto max-w-7xl space-y-6">
      <PageHeader
        ...
        action={
          <div className="flex gap-2">
            <Button variant="secondary" onClick={() => nav({ to: "/app/projects" })}>Progetti</Button>
            {nextAction && (
      ...
      <div className="grid grid-cols-12 gap-4">
        ...
        <Card className="col-span-12 lg:col-span-7">
          <CardHeader title="Today Focus" subtitle={`${todo.length} task prioritari per oggi`} icon={Target}
              ...
              {todo.map((t, i) => (
                ...
                >
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <div className="text-[11.5px] text-muted-foreground">{t.duration} · Output atteso: {t.output}</div>
                  </div>
                  <Pill tone={t.priority === "Alta" ? "warn" : "muted"}>{t.priority}</Pill>
                  <Button variant="secondary" onClick={() => nav({ to: "/app/co-founder" })}>Chiedi all'AI</Button>
        ...
        <Card className="col-span-12 sm:col-span-6 lg:col-span-5">
          <CardHeader title="Startup Health Score" icon={BarChart3} subtitle={`${completed}/${proj.tasks.length} task completati`} />
          ...
          <div className="mt-4 grid grid-cols-2 gap-2 text-[12.5px]">
            {breakdown.map(({ key, value }) => (
              <div key={key} className="rounded-lg border border-border bg-surface/60 p-2">
                <div className="flex justify-between text-muted-foreground"><span>{key}</span><span>{value}</span></div>
                  ...
                  <ul className="mt-3 space-y-1.5 text-[12.5px] text-muted-foreground">
                    {c.items.slice(0, 3).map((i) => (
                      <li key={i.t} className="flex items-center gap-2">
                        <CheckCircle2 className={`h-3 w-3 ${i.done ? "text-success" : "text-muted-foreground/40"}`} /> {i.t}
                      </li>
                    ))}
                  </ul>
                </div>
        ...
        <Card className="col-span-12">
          ...
          <div className="grid gap-2 md:grid-cols-3 text-[13px]">
            {proj.founderAlerts.filter((a) => !a.resolved).slice(0, 3).map((r) => (
              <div key={r.id} className="flex items-start gap-3 rounded-xl border border-border bg-surface/60 p-3">
                <AlertTriangle className={`mt-0.5 h-4 w-4 ${r.severity === "Alta" ? "text-destructive" : r.severity === "Media" ? "text-warning" : "text-muted-foreground"}`} />
...
function EmptyState() {
  return (
    <div className="mx-auto flex max-w-2xl flex-col items-center justify-center py-24 text-center">
      <div className="rounded-2xl bg-brand/10 p-4 text-brand"><Rocket className="h-8 w-8" /></div>
      ...
      <Link to="/onboarding" className="mt-6 inline-flex items-center gap-2 rounded-xl bg-foreground px-5 py-2.5 text-[14px] font-medium text-background hover:opacity-90">
